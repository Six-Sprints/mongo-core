...
public abstract class AbstractCrudController<T extends AbstractMongoEntity, DTO> {
  ...
  @GetMapping("/{slug}")
  public ResponseEntity<RestResponse<DTO>> findBySlug(T user, @PathVariable String slug)
    throws EntityNotFoundException {
    return RestUtil.successResponse(mapper.toDto(service.findBySlug(slug)));
  }

  @PostMapping("/search")
  public ResponseEntity<RestResponse<PageDto<DTO>>> filter(T user, @RequestBody FilterRequestDto filterRequestDto) {
  ...
  public ResponseEntity<RestResponse<List<String>>> getDistinctValues(@RequestParam String column,
    T user, @RequestBody FilterRequestDto filterRequestDto) {
    return RestUtil.successResponse(service.distinctColumnValues(column, filterRequestDto));
  }

  @PostMapping("/delete")
  public ResponseEntity<?> delete(T user, @RequestBody List<String> ids) throws EntityNotFoundException {
    service.delete(ids);
    return RestUtil.successResponse(null);
  }

  @PostMapping(value = "/export", produces = "text/csv")
  public void export(T user,
  ...
  @PostMapping("/import")
  public ResponseEntity<?> uploadFile(T user, @RequestParam(value = "file", required = true) MultipartFile file,
